 ...
 function drawTable(matriz){
   ...
   for(var h = 0; h < mpsoc_x; h++){
     ...
     for(var i = 0; i < mpsoc_y; i++){
       var hasContent = matriz[h][i].enabled;
       if(matriz[h][i].load <= 0){
         hasContent = "noContent";
       }
       if(matriz[h][i].activity == "GMP"){
         hasContent = "GMP";
       }
       if(matriz[h][i] != null){
         $('#table'+id).append("<td id='row"+ h + "_"+ i +"' class='"+ hasContent +"' >"+ matriz[h][i].activity +"</br> Load: " + matriz[h][i].load +"</td>");
       }
     }
   }
 }
